...
for train_index, test_index in kfold.split(x_train): 
    ...
    model = Sequential()
    model.add(Dense(4048, activation=leaky_relu ,input_dim= 6)) 
    ...
    # 3. 컴파일 훈련
    mc = ModelCheckpoint('../data/h5/regressor6_2_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(lr=0.00001), metrics='mae')
    model.fit(x_train1, y_train1, epochs=150, batch_size=128, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
