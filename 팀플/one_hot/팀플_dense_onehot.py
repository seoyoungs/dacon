...
cp = ModelCheckpoint('../data/h5/Dense_0514_2.hdf5', monitor='val_loss', save_best_only=True, verbose=1,mode='auto')
model.compile(loss='mse', optimizer='adam', metrics='mae')
model.fit(x_train, y_train, epochs=50, batch_size=1000, validation_data=(x_val,y_val), callbacks=[reduce_lr,cp] )
...
from tensorflow.keras.models import load_model
# model = load_model('../data/h5/Dense_0514.hdf5', custom_objects={'leaky_relu': tf.nn.leaky_relu})
loss, mae = model.evaluate(x_pred, y_pred, batch_size=1000)
...
df['test'] = y_pred
df.to_csv('../data/csv/teample_dense_1.csv',index=False)
