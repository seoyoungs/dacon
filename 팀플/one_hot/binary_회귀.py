...
# https://stackoverflow.com/questions/55311036/using-binary-encoding-how-to-get-the-original-values-back-from-encoded-values
# https://www.kaggle.com/discdiver/category-encoders-examples
cat_features = ['category', 'dong']
# # count_enc = ce.BinaryEncoder()
# # count_encoded = count_enc.fit_transform(df[cat_features])
# # df = df.join(count_encoded.add_suffix("_count"))
# print(df)

# enc = ce.BinaryEncoder(cols=[cat_features])
...
# df= ce.BinaryEncoder(cols=["category", "dong"])
...
for train_index, test_index in kfold.split(x_train): 
    ...
    cp = ModelCheckpoint('../data/h5/one_hot__dense_1_'+str(num)+'.hdf5', monitor='val_loss', save_best_only=True, verbose=1,mode='auto')
    model.compile(loss='mse', optimizer='adam', metrics='mae')
    model.fit(x_train1, y_train1, epochs=100, batch_size=156, validation_data=(x_val,y_val), callbacks=[es,reduce_lr,cp] )
...
'''
binary
r2 :  [0.7181228618700758, 0.7086859142744081]
RMSE :  [2.0607342440346796, 2.094945857539794]
loss :  [2.766523599624634, 2.5804882049560547]
'''
