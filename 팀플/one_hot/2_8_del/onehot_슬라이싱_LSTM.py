...
# 전체 0 다있는거
SELECT d.date,YEAR,MONTH, d.day, d.time, category, dong,temperature,rain,wind,humidity,VALUE FROM main_data_table AS d INNER JOIN `weather` AS s ON d.date = s.DATE AND d.time = s.time
 ORDER BY DATE, YEAR, MONTH, DAY, TIME, category, dong ASC

# 전체 2~8시 0 없는것
SELECT d.date,YEAR,MONTH, d.day, d.time, category, dong,temperature,rain,wind,humidity,VALUE FROM main_data_table AS d INNER JOIN `weather` AS s ON d.date = s.DATE AND d.time = s.time 
WHERE (d.TIME != 2 AND d.TIME != 3 AND d.TIME != 4 AND d.TIME != 5  AND d.TIME != 6 AND d.TIME != 7 AND d.TIME != 8)
ORDER BY DATE, YEAR, MONTH, DAY, TIME, category, dong ASC
'''
...
x = LSTM(256,activation='tanh')(inputs)
x = Dropout(0.2)(x)
x = Flatten()(x)
...
cp = ModelCheckpoint('../data/h5/Dense_0514_LSTM_4.hdf5', monitor='val_loss', save_best_only=True, verbose=1,mode='auto')
...
loss, mae = model.evaluate(x_pred, y_pred, batch_size=2048)
...
df['test'] = y_pred
df.to_csv('../data/csv/sample_0514_LSTM4.csv',index=False)
