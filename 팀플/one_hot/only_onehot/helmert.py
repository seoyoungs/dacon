...
# df= ce.HelmertEncoder(cols=["category", "dong"], drop_invariant=True)
# df.fit(df[cat_features], df["value"])
# df = pd.concat([df, dfh], axis=1)

# =============== 요것도 낼 돌려보기 ======================
# # https://data-newbie.tistory.com/260
# target_enc = ce.TargetEncoder(cols=cat_features)
# target_enc.fit(df[cat_features], df['value'])
# df = df.join(target_enc.add_suffix("_target"))
...
for train_index, test_index in kfold.split(x_train): 
    ...
    # 2. 모델구성
    model = Sequential()
    model.add(Dense(1024, activation=leaky_relu  ,input_dim= 8)) 
    ...
    cp = ModelCheckpoint('../data/h5/CountEncoder__dense_1_'+str(num)+'.hdf5', monitor='val_loss', save_best_only=True, verbose=1,mode='auto')
    model.compile(loss='mse', optimizer='adam', metrics='mae')
    model.fit(x_train1, y_train1, epochs=50, batch_size=156, validation_data=(x_val,y_val), callbacks=[es,reduce_lr,cp] )
