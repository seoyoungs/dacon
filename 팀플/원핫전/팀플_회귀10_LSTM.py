...
for train_index, test_index in kfold.split(x_train): 
    ...
    model = Sequential()
    model.add(LSTM(512, activation=leaky_relu ,input_shape= (6,1), unroll=False)) 
    ...
    mc = ModelCheckpoint('../data/h5/regressor10_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer=tf.keras.optimizers.Adam(lr=0.002), metrics='mae')
    model.fit(x_train1, y_train1, epochs=20, batch_size=64, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
