...
for train_index, test_index in kfold.split(x_train): 
    ...
    # 3. 컴파일 훈련
    model = load_model('../data/h5/regressor11_3_'+str(num)+'.hdf5', custom_objects={'leaky_relu': tf.nn.leaky_relu})
    mc = ModelCheckpoint('../data/h5/regressor11_3_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer='adam', metrics='mae')
    model.fit(x_train1, y_train1, epochs=30, batch_size=128, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
