...
for train_index, test_index in kfold.split(x_train): 
    ...
    model.add(Conv2D(filters=1024, kernel_size=(2,2), padding='same',
                 strides=(2,2) , input_shape=(6,1,1), activation='tanh'))
    model.add(Conv2D(filters=1024, kernel_size=(2,2), padding='same',
    ...
    # 3. 컴파일 훈련S
    # model = load_model('../data/h5/regressor7_3_'+str(num)+'.hdf5', custom_objects={'leaky_relu': tf.nn.leaky_relu})
    mc = ModelCheckpoint('../data/h5/regressor7_4_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=7, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer=tf.keras.optimizers.RMSprop(lr=2e-5), metrics='mae')
    model.fit(x_train1, y_train1, epochs=50, batch_size=128, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
