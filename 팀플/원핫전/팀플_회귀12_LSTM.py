...
# 3. 컴파일 훈련
model = load_model('../data/h5/regressor12_0.hdf5', custom_objects={'leaky_relu': tf.nn.leaky_relu})
# # mc = ModelCheckpoint('../data/h5/regressor12_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
# es= EarlyStopping(monitor='val_loss', patience=8)

# reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

# model.compile(loss='mse', optimizer=tf.keras.optimizers.Nadam(lr=4e-4, clipnorm=1), metrics='mae')
# model.fit(x_train1, y_train1, epochs=30, batch_size=64, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
