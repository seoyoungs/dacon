...
for train_index, test_index in kfold.split(x_train): 
    ...
    mc = ModelCheckpoint('../data/h5/regressor11_2_'+str(num)+'.hdf5',save_best_only=True, verbose=1)
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer='AdaDelta', metrics='mae')
    model.fit(x_train1, y_train1, epochs=35, batch_size=64, validation_data=(x_val,y_val), callbacks=[es,reduce_lr, mc] )
