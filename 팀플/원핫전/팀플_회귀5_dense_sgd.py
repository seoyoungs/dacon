...
for train_index, test_index in kfold.split(x_train): 
    ...
    modelpath = '../data/modelCheckpoint/regressor5_'+str(num)+'.hdf5'
    es= EarlyStopping(monitor='val_loss', patience=8)

    reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, factor=0.5, verbose=1)

    model.compile(loss='mse', optimizer=tf.keras.optimizers.RMSprop(0.001), metrics='mae')
    model.fit(x_train1, y_train1, epochs=100, batch_size=128, validation_data=(x_val,y_val), callbacks=[es,reduce_lr] )
...
'''
RMSE :  3.8820650657918705
R2 :  -0.0003258638353409449
r2 :  [-0.00036723615506439877, -0.0003179871882323937, -0.0008810913581112079, -0.0004634577893189995, -0.0003258638353409449]8353409449]
RMSE :  [3.8821453438203952, 3.882049781913949, 3.883142279947596, 3.882332043951015, 3.8820650657918705]
loss :  [12.445134162902832, 12.546205520629883, 12.167837142944336, 12.051797866821289, 12.580092430114746]        
7070.091785초 걸렸습니다.
