...
# ======================== Dacon Dataset Load ==========================
labels_df = pd.read_csv('C:/data/dacon_mnist2/dirty_mnist_answer.csv')[:] # 전체 다 사용
...
#     model = nn.DataParallel(model)
    ...
    epochs=30
    batch_size=80         # 자신의 VRAM에 맞게 조절해야 OOM을 피할 수 있습니다.
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/data/dacon_mnist2/model_mnist/EfficientNetB3-fold{}.pt'.format(fold))
...
submission.to_csv('C:/data/dacon_mnist2/anwsers/0225_1_base.csv', index=False)
