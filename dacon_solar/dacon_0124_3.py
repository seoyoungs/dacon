...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y1_0124_3_2_q_{i:.1f}.hdf5'
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y2_0124_3_2_q_{i:.1f}.hdf5'
    cp = ModelCheckpoint(filepath_cp, monitor='val_loss', mode='min', verbose=1, save_best_only=True, save_weights_only=True)
    model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('C:/data/dacon_data/sub_0124_5.csv', index = False)
