...
def preprocess_data(data, is_train=True):
    ...
    temp = data.copy()
    temp = temp[['Hour','TARGET','GHI','T-Td','DHI','DNI','RH', 'T']] # 상관관계 낮은 WS 삭제
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y1_2_quantile_{i:.1f}.hdf5'
...
for i in q:
    model = modeling()
    filepath_cp = f'C:/data/modelCheckpoint/dacon_y2_2_quantile_{i:.1f}.hdf5'
    # cp = ModelCheckpoint(filepath_cp,save_best_only=True,monitor = 'val_loss')
    # model.compile(loss = lambda y_true,y_pred: quantile_loss(i,y_true,y_pred), optimizer = 'adam', metrics = [lambda y,y_pred: quantile_loss(i,y,y_pred)])
...
submission.to_csv('C:/data/dacon_data/sub_0122_5.csv', index = False)
