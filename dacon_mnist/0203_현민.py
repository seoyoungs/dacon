...
es = EarlyStopping(patience=130, verbose=1)
val_loss_min = []
val_acc_max = []
result = 0
nth = 0
...
for train_index, test_index in skf.split(train2, train['digit']) : # >>> x, y
    path = 'C:/data/modelCheckpoint/0205_6_hyunmin_best_mc.hdf5'
    ...
    x_train, x_valid, y_train, y_valid = train_test_split(x_train, y_train, train_size=0.85, shuffle=True, random_state=47)
    ...
    pred_generator = idg2.flow(test2, shuffle=False, batch_size=16)
    ...
    result += model.predict_generator(pred_generator, verbose=True)/20
...
print(sub)
sub.to_csv('C:/data/dacon_mnist/answer/please4.csv', index=False)
