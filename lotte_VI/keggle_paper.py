# https://www.kaggle.com/ignatovdv/cnn-pytorch-simpsons
# https://www.kaggle.com/cla0709/simpson-characters-classification/data
...
mc = ModelCheckpoint('C:/data/h5/lotte_0317_1.h5',save_best_only=True, verbose=1)



train_generator = idg.flow(x_train,y_train,batch_size=20)
# seed => random_state
...
model.compile(loss='sparse_categorical_crossentropy', optimizer=Adam(lr=1e-5,epsilon=None),
                metrics=['acc'])
learning_history = model.fit_generator(train_generator,epochs=120, 
    validation_data=valid_generator, callbacks=[early_stopping,lr,mc])

# predict
model.load_weights('C:/data/h5/lotte_0317_1.h5')
result = model.predict(test_generator,verbose=True)

print(result.shape)
sub = pd.read_csv('C:/data/LPD_competition/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('C:/data/csv/lotte0317_1.csv',index=False)
